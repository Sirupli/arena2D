...
DISCOUNT_FACTOR = 0.95		# discount factor for reward estimation (often denoted by gamma)
...
EPSILON_MAX_STEPS = 2*10**5	# how many steps until epsilon reaches minimum
...
class Agent:
	def __init__(self, device, model_name, num_obs, num_envs, num_threads, training_data_path):
		...
		self.tensor_state_buffer = torch.zeros(MEMORY_SIZE, self.num_observations ,dtype=torch.float).to(self.device)# state
		...
		self.tgt_net = fc.FC_DQN(self.num_observations, NUM_ACTIONS)
	...
	def post_step(self, new_observation, reward, is_done, mean_reward, mean_success, mean_collision):
		self.start_gpu_measure()
		feature=utils.FeatureExtraction(new_observation)
		new_observation=feature.getDistanceOnlyObservation()
		self.last_observation = new_observation
		idx = self.frame_idx%MEMORY_SIZE
		if is_done: # save next state if done, because next pre_step will have different state
			self.tensor_state_buffer[(idx+1)%MEMORY_SIZE] = torch.FloatTensor(self.last_observation)
