...
EPSILON_MAX_STEPS = 3*10**5	# how many steps until epsilon reaches minimum influence training
	...
	def pack_episodes(self, indicies):
		...
		for i in indicies:
			...
			if start_index < 0: # wrap around -> two part sequence
				...
				sequence = torch.narrow(self.tensor_state_buffer, dim=0, start=int(start_index), length=250).flip([0])
				#print(sequence)
