...
class Agent:
	def __init__(self, device, model_name, num_obs, num_envs, num_threads, training_data_path):
		...
		self.tensor_state_buffer = torch.zeros(MEMORY_SIZE, self.num_observations+additional_state ,dtype=torch.float).to(self.device)# state
		...
		self.tgt_net = gru.GRUModel(self.num_observations+additional_state, NUM_ACTIONS)
	...
	def post_step(self, new_observation, reward, is_done, mean_reward, mean_success, mean_collision):
		...
		new_observation=feature.getObservation()
		self.last_observation = [self.last_reward,self.last_action]+new_observation
		idx = self.frame_idx%MEMORY_SIZE
